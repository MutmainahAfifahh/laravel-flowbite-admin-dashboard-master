...
<div class="p-4 bg-white block border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
    <div class="w-full mb-4">
        <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white mb-4">Transaksi Barang Keluar (Pengeluaran Barang)</h1>
        ...
        <!-- Form Catat Barang Keluar -->
        <div class="p-4 mb-6 bg-gray-50 rounded-lg dark:bg-gray-700 border border-gray-200 dark:border-gray-600">
            <h2 class="mb-4 text-lg font-bold text-gray-900 dark:text-white">Catat Pengeluaran Barang Keluar</h2>
            ...
            <form action="{{ route('stock-transactions.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <!-- 2. Jumlah Qty -->
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jumlah (Qty Keluar) <span class="text-red-500">*</span></label>
                    ...
                    <!-- 3. Tanggal Keluar -->
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tanggal Pengeluaran <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        ...
                        <select name="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-800 dark:border-gray-600 dark:text-white" required>
                            <option value="Dikeluarkan" {{ old('status') == 'Dikeluarkan' ? 'selected' : '' }}>Dikeluarkan (Completed)</option>
                    ...
                    <div class="md:col-span-2">
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Catatan Pengeluaran</label>
                        <input type="text" name="notes" value="{{ old('notes') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-800 dark:border-gray-600 dark:text-white" placeholder="Contoh: Pengeluaran untuk toko cabang / pesanan pelanggan">
                    </div>
                </div>

                <div class="text-right">
                    <button type="submit" class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-red-600 dark:hover:bg-red-700">
                        - Catat Barang Keluar
                    </button>
                </div>
            </form>
                ...
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                    @forelse($transactions as $index => $tx)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-4 py-4 text-center font-bold text-red-600 dark:text-red-400">-{{ $tx->quantity }}</td>
                            ...
                            <td class="px-4 py-4 text-center">
                                <form action="{{ route('stock-transactions.destroy', $tx->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus catatan barang keluar ini?')" class="inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">Belum ada riwayat transaksi barang keluar.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
